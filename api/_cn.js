...
async function cnFetch(path, body) {
  ...
  try {
    ...
    if (!r.ok) {
      console.error(`CN ${path} → ${r.status}: ${text.slice(0, 300)}`);
    ...
    console.error(`CN ${path} failed:`, e.message);
...
function cnProfileMatch(profile) {
  return cnFetch("/profiles/match", profile);
}
