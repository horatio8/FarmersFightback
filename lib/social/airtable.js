...
// Where the log lived before the split. Read-only: nothing writes here again.
const LEGACY_EVENTS = { base: AIRTABLE_BASE_ID, table: TABLES.EVENTS };
...
async function select(table, formula, fields, maxRecords) {
  ...
  if (table !== TABLES.EVENTS || !EVENTS_SPLIT) return records;
  if (maxRecords && records.length >= maxRecords) return records;

  const legacy = await at('GET', LEGACY_EVENTS, '', null, query);
  const all = records.concat(legacy.records || []);
  // Each base honoured maxRecords on its own, so the union can hold up to
  // double the cap. Trim so the promise callers see is kept.
