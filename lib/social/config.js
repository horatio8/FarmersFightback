...
const EVENTS_BASE_ID = process.env.AIRTABLE_EVENTS_BASE_ID || 'appxb9ykk2eXbuoIB';

module.exports = {
  AIRTABLE_BASE_ID,

  // The Events log lives outside the main base as of 18 Aug 2026. The main
  // base hit its record ceiling and every logEvent() write started failing,
  // taking petition signups down with it. Airtable's record limit is per
  // base, so each new base buys a fresh allowance at no extra cost.
  //
  // It is a CHAIN of slices, not one base: the full history overflowed slice
  // 1 (appE8OEBzFLzOfdMm) the same day. Writes go to the live slice only;
  // reads union the live slice with EVENTS_HISTORY. When the live slice
  // fills, mint the next base, append the current live pair to the history,
  // and point these two env vars at the new one.
  //
  ...
  EVENTS_BASE_ID,
  EVENTS_TABLE_ID: process.env.AIRTABLE_EVENTS_TABLE_ID || 'tblrzZgWgWbRXnaXk',

  // Retired read-only slices of the log, oldest first. Table ids are per
  // base, so each entry carries its own.
  EVENTS_HISTORY: [
    { base: AIRTABLE_BASE_ID, table: 'tblhCWL3mckJl6YQ7' },
    { base: 'appE8OEBzFLzOfdMm', table: 'tblbI6YkuKYAKY3Li' },
  ].filter((h) => h.base !== EVENTS_BASE_ID),
  ...
  TABLES: {
    ...
    // that module needs to know the log moved (or how many slices it spans).
